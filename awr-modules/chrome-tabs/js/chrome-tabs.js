((window, factory) => {
  ...
  class ChromeTabs {
    ...
    createNewTabEl(closable = true) {
      const div = document.createElement('div')
      div.innerHTML = tabTemplate;
      return div.firstElementChild
    }

    addTab(tabProperties, { animate = true, background = false, closable = true } = {}) {
      const tabEl = this.createNewTabEl()
      if (!closable) {
        tabEl.querySelector('.chrome-tab-close').style.display = 'none';
      } else {
        tabEl.querySelector('.chrome-tab-close').style.display = 'block';
      }
    ...
    updateTab(tabEl, tabProperties) {
      ...
      } else if (tabProperties.icon) {
        faviconEl.innerHTML = '';
        const i = document.createElement('i');
        i.className = tabProperties.icon;
        faviconEl.appendChild(i);
        faviconEl.removeAttribute('hidden', '')
      } else {
